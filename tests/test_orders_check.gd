...
func _place(st: BattleState, owner: int, row: int, col: int, card_id: String, air: bool = false) -> BattleState.UnitData:
	...
	if card_data != null:
		...
		u.abilities = card_data.abilities.duplicate()
...
func _test_orders() -> void:
	...
	print("[DBG] O1 st2=", st2, " K=", st.players[0].resources["K"], " in_hand=", st.players[0].hand.has("iron_cross"), " type=", CardDataLoader.cards.get("iron_cross").type)
...
func _test_triggers() -> void:
	...
	_place(st, 1, 2, 2, "infantry_01")  # 目标先在场：on_deploy 在部署瞬间结算
	var st2 := GameLogic.deploy_unit(st, 0, "charron_1906", 0, 0)
	_check(st2 != null, "T1 deploy ok")
	if st2 != null:
		var enemy_u: BattleState.UnitData = st2.board.get_unit(2, 2)
		_check(enemy_u != null and enemy_u.suppressed, "T1b on_deploy suppress applied")
	# T2 on_death 转化（戈塔 G.III → G.IV）
	st = _fresh()
	var g3 := _place(st, 0, 2, 2, "gota_g3", true)
	g3.defense = 1
	g3.max_defense = 1
	_place(st, 1, 2, 3, "fighter_01", true)
	...
	print("[DBG] T6 reg2=%s def=%s trig=%s" % [reg2, reg2.defense if reg2 != null else -1, CardDataLoader.cards.get("infantry_regiment_8").triggers])
	...
	_check(st9 != null and st9.players[0].hand.is_empty(), "T8 no iron cross without austro deploy")
	...
	stealth.attack = 0
	_place(st, 1, 2, 3, "infantry_01")
	var st10 := GameLogic.attack_unit(st, 0, 2, 2, 2, 3)
	var log_types: Array = []
	if st10 != null:
		for a in st10.action_log:
			log_types.append(a.get("type"))
	print("[DBG] T9 st10=", st10 != null, " log_types=", log_types, " trig_keys=", CardDataLoader.cards.get("rolls_royce_1914").triggers.keys())
	_check(st10 != null, "T9 attack resolves")
	if st10 != null:
		var stealth_new: BattleState.UnitData = st10.board.get_unit(3, 2, "air")
		_check(stealth_new != null and stealth_new.revealed, "T9b enemy stealth unit revealed by damage dealer")
	...
	# T11 嘲讽（L44 只能被攻击）
	st = _fresh()
	var l44 := _place(st, 1, 2, 1, "zeppelin_l44", true)
	...
	_check(targets.size() == 1 and targets[0] == Vector2i(2, 1), "T11 taunt forces targeting L44 only")
	# T12 cannot_be_attacked_by:fighter（L70）
	st = _fresh()
	_place(st, 1, 2, 0, "zeppelin_l70", true)
	var g_target := _place(st, 1, 2, 3, "infantry_01")
	var my_f2 := _place(st, 0, 2, 2, "fighter_01", true)
	var dbg_me2: BattleState.UnitData = st.board.get_unit(2, 2, "air")
	print("[DBG] T12 atk_placed=", dbg_me2 != null)
	targets = GameLogic.get_valid_attack_targets(st, 0, 2, 2)
	_check(targets.size() == 1 and targets[0] == Vector2i(2, 3), "T12 L70 immune to fighter, only ground target listed")
	...
	if st13 != null:
		...
		_check(d_after == null, "T13 ignore_fort: full 6 damage destroys def-6 unit")
	...
	print("[DBG] T15 eh2=%s def=%s phase=%s" % [eh2, eh2.defense if eh2 != null else -1, st16.phase])
