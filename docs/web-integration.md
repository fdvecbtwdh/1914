...
- **代理**：网站侧 GitHub 同步可配置代理（`.env` 的 `GITHUB_PROXY`，如 Clash 的 `http://127.0.0.1:7890`），仅影响网站自身的同步流量，游戏服无需代理。
...
3. **卡牌性质（tag，网站侧字段）**：网站按**游戏版本号**自动判定"正式/测试"性质——
   版本号 `v0.*` 阶段导入的官方卡一律为测试卡，`v1.0` 起才判为正式卡；
   游戏 `id` 含 `test` 或名称含 `测试` 的显式测试卡在任何版本都保持测试。
   版本号取网站 `.env` 的 `GAME_VERSION`，该字段不在游戏 JSON 中，无需游戏侧维护；
   游戏发布 v1.0 时提醒网站管理员更新版本号并重跑导入（后台"导入官方卡牌"或 `manage.py import-cards`）。
4. **开发注意事项**：
